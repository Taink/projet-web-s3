...
const createUserTable = (db) => {
   let stmt= db.prepare(`CREATE TABLE IF NOT EXISTS users ( 
...
const confirmUser = (db,name,password)=>{ //return true si l'utilisateur existe et que le password est bon
    let stmt= db.prepare(`SELECT name Name,password Password FROM users WHERE name=?`);
